...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        input_dim = self.n_features * self.embed_size
        w1 = torch.empty(self.hidden_size, input_dim)
        ...
        w2 = torch.empty(self.n_classes, self.hidden_size)
        nn.init.xavier_uniform_(w2)
        b2 = torch.empty(self.n_classes)
        nn.init.uniform_(b2)

        self.hidden_to_logits_weights = nn.Parameter(w2)
    ...
    def forward(self, w):
        ...
        hidden = F.relu(F.linear(x, self.embed_to_hidden_weight, self.embed_to_hidden_bias))
        hidden = self.dropout(hidden)
        logits = F.linear(hidden, self.hidden_to_logits_weights, self.hidden_to_logits_bias)
        ### TODO:
