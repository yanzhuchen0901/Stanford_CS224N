...
class CausalAttention(nn.Module):
    ...
    def forward(
        ...
        attention_scores = (q @ k.transpose(-2, -1)) / math.sqrt(self.d_attention)

        mask = self.causal_mask[:, :, : attention_scores.shape[-2], : attention_scores.shape[-1]]
...
class Transformer(nn.Module):
    ...
    def get_loss_on_batch(
        ...
        loss = F.cross_entropy(logits.view(-1, logits.shape[-1]), input_ids_target.reshape(-1))
        return loss
